...
find_seven_zip() {
    if command -v 7z >/dev/null 2>&1; then
        seven_zip_exe="$(command -v 7z)"
        return
    fi

    if command -v 7za >/dev/null 2>&1; then
        seven_zip_exe="$(command -v 7za)"
        return
    fi

    echo "未找到 7-Zip，请先安装 7z 或 7za"
    exit 1
}

pack_burn_files() {
    local archive_name="H6_APP_${version_info}.7z"
    ...
    burn_package_archive="build/${archive_name}"
    ...
    rm -f "$burn_package_archive"
    echo "打包烧录文件: $burn_package_archive"
    (
        cd build
...
if ! branch_name=$(git rev-parse --abbrev-ref HEAD 2>/dev/null) ||
        [[ -z "$branch_name" ]]; then
    branch_name="unknown"
fi
