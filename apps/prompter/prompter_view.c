...
bool prompter_font_init_from_config(void) {
    if (!prompter_config_load_runtime()) {
        return false;
    }
...
/**
 * @brief 判断相邻 seekTo 窗口是否满足对应方向的滚动条件。
 *
 * 后划要求新窗口起点落在上一窗口内；前滑要求新窗口终点落在上一窗口内。
 *
 * @param previous 上一次逻辑窗口。
 * @param current 本次逻辑窗口。
 * @return `true` 表示两帧足够接近，可以合并后滚动。
 */
static bool prompter_external_view_can_animate(const prompter_external_view_t* previous,
                                               const prompter_external_view_t* current) {
    ...
    previous_end = (uint64_t)previous->offset + previous->length;
    current_end = (uint64_t)current->offset + current->length;
    if (current->offset > previous->offset) {
        return (uint64_t)current->offset < previous_end;
    }
    if (current->offset < previous->offset) {
        return current_end > previous->offset && current_end <= previous_end;
    }
    return false;
}
...
void prompter_text_apply_external_view(const prompter_external_view_t* view) {
    ...
    prompter_external_view_t current_view = {0};
    ...
    current_view = *view;
    if (current_view.length > s_file_size - current_view.offset) {
        current_view.length = s_file_size - current_view.offset;
    }
    current_end = current_view.offset + current_view.length;
    prompter_update_progress_by_offset(current_end);
    if (current_view.length == 0) {
        free_page_buffer();
        paged_text_set_visible_text(text_view, "");
        paged_text_set_highlight_window(text_view,
                                        current_view.top_mask_height,
                                        current_view.bottom_mask_height);
        s_last_external_view = current_view;
        s_last_external_view_valid = true;
        ...
        floatair_info("prompter apply external view cost %lu us/%lu ms, result=empty, offset=%lu, length=%lu, read=%lu",
                      ...
                      (unsigned long)current_view.length);
        return;
    }

    if (s_last_external_view_valid &&
        s_last_external_view.offset == current_view.offset &&
        s_last_external_view.length == current_view.length &&
        s_file_buf != NULL) {
        paged_text_set_highlight_window(text_view,
                                        current_view.top_mask_height,
                                        current_view.bottom_mask_height);
        s_last_external_view = current_view;
        cost_time_us = (uint32_t)GetTimeUs() - start_time_us;
        floatair_info("prompter apply external view cost %lu us/%lu ms, result=same, offset=%lu, length=%lu",
                      (unsigned long)cost_time_us,
                      (unsigned long)(cost_time_us / 1000U),
                      (unsigned long)view->offset,
                      (unsigned long)view->length);
    ...
    animate = current_view.duration_ms > 0 &&
              s_last_external_view_valid &&
              prompter_external_view_can_animate(&s_last_external_view, &current_view);
    read_offset = current_view.offset;
    read_end = current_end;
    if (animate) {
        uint32_t previous_end = s_last_external_view.offset + s_last_external_view.length;

        read_offset = LV_MIN(s_last_external_view.offset, current_view.offset);
        read_end = LV_MAX(previous_end, current_end);
        from_offset = s_last_external_view.offset - read_offset;
        to_offset = current_view.offset - read_offset;
    }
    read_len = read_end - read_offset;

    if (s_file_buf == NULL || s_page_offset != read_offset || s_page_len != read_len) {
        if (!prompter_read_text_range(read_offset, read_len, &page_buf)) {
            return;
        }
        free_page_buffer();
        s_file_buf = page_buf;
        s_page_offset = read_offset;
        s_page_len = read_len;
    }

    if (animate) {
        paged_text_set_visible_text_animated(text_view,
                                             s_file_buf,
                                             from_offset,
                                             to_offset,
                                             current_view.duration_ms);
    } else {
    ...
    cost_time_us = (uint32_t)GetTimeUs() - start_time_us;
    floatair_info("prompter apply external view cost %lu us/%lu ms, result=%s, offset=%lu, length=%lu, duration=%lu ms, read_offset=%lu, read=%lu, from=%lu, to=%lu",
                  (unsigned long)cost_time_us,
                  (unsigned long)(cost_time_us / 1000U),
                  animate ? "animate" : "jump",
                  (unsigned long)view->offset,
                  (unsigned long)view->length,
                  (unsigned long)current_view.duration_ms,
...
static void touch_event_handle(lv_event_t* event) {
    ...
    if (code == LV_EVENT_DCLICKED) {
        if (prompter_report_double_click_enabled()) {
...
bool prompter_on_fontconfig_changed(void) {
    prompter_external_view_t current_view = s_last_external_view;
    bool restore_current_view = s_last_external_view_valid;
    ...
    paged_text_set_font_info(s_text_preview_ui.text_view, &prompter_font_info);
    if (restore_current_view) {
        prompter_text_apply_external_view(&current_view);
    }
    return true;
}
