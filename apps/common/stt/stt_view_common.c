...
static void stt_view_deferred_update_async_cb(void* user_data) {
    stt_view_deferred_update_t* state = (stt_view_deferred_update_t*)user_data;
    stt_view_deferred_update_cb_t callback = NULL;
    void* callback_user_data = NULL;
    const char* name = NULL;
    uint32_t merged = 0;
    uint32_t update_start_tick = lv_tick_get();
    ...
    callback_user_data = state->user_data;
    name = state->name;
    merged = state->merged;
    state->pending = false;
    state->merged = 0;
    ...
    floatair_info("%s coalesced UI update cost=%lu ms merged=%lu",
                  name != NULL ? name : "stt",
                  (unsigned long)lv_tick_elaps(update_start_tick),
                  (unsigned long)merged);
}

void stt_view_request_deferred_update(stt_view_deferred_update_t* state,
                                      const char* name,
                                      stt_view_deferred_update_cb_t callback,
                                      void* user_data) {
    ...
    if (lv_async_call(stt_view_deferred_update_async_cb, state) != LV_RESULT_OK) {
        state->pending = false;
        state->merged = 0;
        callback(user_data);
    }
...
void stt_view_cancel_deferred_update(stt_view_deferred_update_t* state) {
    if (state == NULL) {
        return;
    }
    if (state->pending) {
        (void)lv_async_call_cancel(stt_view_deferred_update_async_cb, state);
    }
    state->pending = false;
    state->merged = 0;
}
