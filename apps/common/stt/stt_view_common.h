...
typedef void (*stt_view_deferred_update_cb_t)(void* user_data);
...
typedef struct {
    ...
    void* user_data;
} stt_view_deferred_update_t;

/**
 * @brief 请求在下一次 LVGL 节拍合并执行一次页面更新。
 *
 * @param state 页面持有的静态合并状态。
 * @param name 页面名称，用于性能日志。
 * @param callback 实际页面更新回调。
