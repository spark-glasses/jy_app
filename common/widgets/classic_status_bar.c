...
static int32_t status_bar_get_right_reserved_width(const status_bar_data_t* data) {
    ...
    if (data->headset_icon != NULL) {
        headset_width = STATUS_BAR_IMG_W + STATUS_BAR_ITEM_SPACING;
    }

    return STATUS_BAR_IMG_W + battery_label_width + headset_width +
           STATUS_BAR_ITEM_SPACING * 2;
}
...
static void status_bar_align_right_widgets(status_bar_data_t* data) {
    ...
    if (data->headset_icon != NULL) {
        lv_obj_align_to(data->headset_icon,
                        data->battery_label != NULL ? data->battery_label : data->battery_icon,
                        LV_ALIGN_OUT_LEFT_MID,
                        -STATUS_BAR_ITEM_SPACING,
                        0);
    }
...
lv_obj_t* status_bar_create_with_pos(lv_obj_t* parent, int32_t width, const lv_font_t* font, status_bar_widget_pos_t pos) {
...
#if PRODUCT_STATUS_BAR_HEADSET
        headset_icon = lv_image_create(status_bar);
    ...
    if (pos == STATUS_BAR_POS_BOTTOM) {
        ...
        status_bar_update_headset_state(status_bar, false, false);
...
void status_bar_set_position(lv_obj_t* status_bar, status_bar_widget_pos_t pos) {
    if (status_bar == NULL ||
        (pos != STATUS_BAR_POS_TOP && pos != STATUS_BAR_POS_BOTTOM)) {
        return;
    }

    lv_obj_align(status_bar,
                 (pos == STATUS_BAR_POS_TOP) ? LV_ALIGN_TOP_MID : LV_ALIGN_BOTTOM_MID,
                 0,
                 0);
}
...
#if PRODUCT_STATUS_BAR_HEADSET
    status_bar_data_t* data = NULL;
    const char* src = NULL;

    if (status_bar == NULL) {
        return;
    }
    data = (status_bar_data_t*)lv_obj_get_user_data(status_bar);
    if (data == NULL || data->headset_icon == NULL) {
        return;
    }

    if (left_connected && right_connected) {
        src = UI_RES_IMAGE_AIRPODS;
    } else if (left_connected) {
        src = UI_RES_IMAGE_AIRPODS_LEFT;
    } else if (right_connected) {
        src = UI_RES_IMAGE_AIRPODS_RIGHT;
    }

    if (src != NULL) {
        const void* current_src = lv_image_get_src(data->headset_icon);
        bool same_src = false;
        if (current_src != NULL && lv_image_src_get_type(current_src) == LV_IMAGE_SRC_FILE) {
            same_src = strcmp((const char*)current_src, src) == 0;
        }
        if (!same_src) {
            lv_image_set_src(data->headset_icon, src);
        }
        lv_obj_remove_flag(data->headset_icon, LV_OBJ_FLAG_HIDDEN);
    } else {
        lv_obj_add_flag(data->headset_icon, LV_OBJ_FLAG_HIDDEN);
    }
#else
    (void)status_bar;
    (void)left_connected;
    (void)right_connected;
#endif
}
