                                 ...
                                 uint32_t unicode_letter_next)
{
    ...
    if(entry == NULL) {
        LV_LOG_ERROR("cache_size: %u", dsc->cache_size);
...
{
    /*Init cache*/
    LV_LOG_ERROR("cache_size: %" PRIu32, (uint32_t)dsc->cache_size);
...
{
    LV_UNUSED(data_size);
    LV_LOG_ERROR("cache_size: %u font_size: %ld", cache_size, font_size);
